...
(function(global){
  ...
  checkAndHandleTalentLoss(student) {
              ...
              if (getRandom() < prob) {
                ...
                try{ 
                  if(window.showEventModal) {
                    window.showEventModal({ 
                      name: '负面天赋获得', 
                      description: `由于长期处于高压状态，${student.name} 产生了负面赛场心态，获得了负面天赋：【${tDef.name}】！`, 
                      week: window.game ? window.game.week : undefined 
                    });
                  }
                }catch(e){}
    ...
    registerDefaultTalents(game, utils){
      ...
      // 伽罗瓦：遇到数学题时可能触发，数学知识与工程设计能力 +50%
      this.registerTalent({
        name: '伽罗瓦',
        description: '遇到数学题时有概率爆发，数学知识与工程设计能力临时+50%。',
        color: '#FF9800',
        prob: 0.05,
        beneficial: true,
        handler: function(student, eventName, ctx){
          try{
            ...
            if(getRandom() < 0.25){
              ...
              if(!student._talent_backup['伽罗瓦']){
                ...
                return '伽罗瓦发动：数学与工程设计能力 +50%（本题临时）';
              }
            }
      ...
      // 爆发型：连续换题 2 次后下一题发动，所有知识点与工程设计能力提高100%
      this.registerTalent({
        name: '爆发型',
        description: '连续换题两次后，下一题有较大概率爆发，知识点与工程设计能力临时翻倍。',
        color: '#E91E63',
        prob: 0.05,
        beneficial: true,
        handler: function(student, eventName, ctx){
            ...
            if(eventName === 'contest_select_problem'){
              ...
              if(cnt >= 2 && getRandom() < 0.5){
                if(!student._talent_backup['爆发型']){
                  ...
                  return '爆发型发动：所有知识点与工程设计能力翻倍（本题临时）';
                }
              }
            }
            // reset on solve
      ...
      // 心态稳定：已解题数 >=3 时赛场心态提高50%
      this.registerTalent({
        name: '心态稳定',
        description: '比赛中解题数达到 3 题后，赛场心态提升 50%。',
        color: '#2196F3',
        prob: 0.10,
        beneficial: true,
        handler: function(student, eventName, ctx){
      ...
      // 激进：工程设计能力 +100%，赛场心态 -50%，只尝试当前题目的最后一档
      this.registerTalent({
        name: '激进',
        description: '激进做题风格：工程设计能力大幅提升但赛场心态下降，只尝试最后一档部分分。',
        color: '#F44336',
        prob: 0.05,
        beneficial: true,
        handler: function(student, eventName, ctx){
          try{
            ...
            if(eventName === 'contest_start'){
                ...
                return '激进风格：工程设计能力翻倍，赛场心态减半（本场临时）';
              }
            }
        ...
        this.registerTalent({
          name: '硬件攻坚者',
          description: '在模拟赛选题时若任务包含“硬件设计”标签，本题临时让对应模块效率翻倍（x2）。',
          color: '#00BCD4',
          prob: 0.08,
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              ...
              // 确保不会重复备份
              if(!student._talent_backup['硬件攻坚者']){
                student._talent_backup['硬件攻坚者'] = { knowledge_ds: Number(student.knowledge_ds || 0) };
                student.knowledge_ds = Number(student.knowledge_ds || 0) * 2.0;
                student._talent_state['硬件攻坚者'] = true;
                return '硬件攻坚者发动：硬件设计模块效率翻倍（本题临时）';
              }
            }catch(e){ console.error('硬件攻坚者 天赋错误', e); }
            return null;
          }
        });
        ...
        this.registerTalent({
          name: '传感直觉',
          description: '在模拟赛选题时若任务含“传感与识别”标签，有 30% 概率临时增加该模块 +60%，并让工程设计 +20%。',
          color: '#3F51B5',
          prob: 0.05,
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              ...
              if(getRandom() < 0.30){
                if(!student._talent_backup['传感直觉']){
                  student._talent_backup['传感直觉'] = { knowledge_graph: Number(student.knowledge_graph || 0), thinking: Number(student.thinking || 0) };
                  student.knowledge_graph = Number(student.knowledge_graph || 0) * 1.6;
                  student.thinking = clamp(Number(student.thinking || 0) * 1.2);
                  student._talent_state['传感直觉'] = true;
                  return '传感直觉发动：传感与识别 +60%，工程设计 +20%（本题临时）';
                }
              }
            }catch(e){ console.error('传感直觉 天赋错误', e); }
            return null;
          }
        });
        ...
        this.registerTalent({
          name: '赛场狂热',
          description: '比赛前半段（前 50% 题目）工程设计 +25%，后半段赛场心态临时值衰减至 0.8 倍。',
          color: '#FF5722',
          prob: 0.08,
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              ...
              if(eventName === 'contest_select_problem'){
                ...
                if(pid < half && !student._talent_backup['赛场狂热_first']){
                  ...
                  return '赛场狂热（前半段）发动：工程设计 +25%（赛中临时）';
                }
                // 后半段：赛场心态（per-contest constmental）衰减至 0.8 倍（只需第一次进入后半段时应用）
                if(pid >= half && !student._talent_backup['赛场狂热_second']){
                  ...
                  return '赛场狂热（后半段）发动：赛场心态下降至 80%（赛中临时）';
                }
              }
        ...
        this.registerTalent({
          name: '灵感跃迁',
          description: '每次切换任务，工程设计 +10%，可叠加（最多 3 层，模拟赛中临时生效）。',
          color: '#009688',
          prob: 0.08,
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              ...
              if(!student._talent_backup['灵感跃迁']){
                student._talent_backup['灵感跃迁'] = { thinking: Number(student.thinking || 0) };
              }
              const newLayers = Math.min(3, layers + 1);
              student._talent_state._jump_layers = newLayers;
              // apply multiplier: each层 +10% 叠加
              student.thinking = clamp(Number(student._talent_backup['灵感跃迁'].thinking || student.thinking || 0) * Math.pow(1.1, newLayers));
              student._talent_state['灵感跃迁'] = true;
              return `灵感跃迁发动：切换次数叠加至 ${newLayers} 层，工程设计 +${(newLayers*10)}%（赛中临时）`;
            }catch(e){ console.error('灵感跃迁 天赋错误', e); }
            return null;
          }
        });
        ...
        this.registerTalent({
          name: '慢热',
          description: '比赛初期能力受限，但随着时间推移逐渐进入状态。前半场工程设计与编程-20%，后半场+20%。',
          color: '#90A4AE',
          prob: 0.08,
          kind: 'positive',
          beneficial: true,
        ...
        this.registerTalent({
          name: '虎头蛇尾',
          description: '比赛开始时状态极佳，但耐力不足。前半场工程设计与赛场心态+30%，后半场所有能力-20%。',
          color: '#FF6F00',
          prob: 0.08,
          kind: 'negative',
          beneficial: false,
        ...
        this.registerTalent({
          name: '读题专家',
          description: '擅长理解题意，在工程设计检定上占有优势。',
          color: '#00897B',
          prob: 0.10,
          kind: 'positive',
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              ...
              return { action: 'reduce_difficulty', amount: 0.10, message: '读题专家：工程设计难度-10%' };
        ...
        this.registerTalent({
          name: '键盘侠',
          description: '嵌入式开发速度极快，在代码检定上占有优势。',
          color: '#5E35B1',
          prob: 0.10,
          kind: 'positive',
          beneficial: true,
        ...
        this.registerTalent({
          name: '字符串魔法师',
          description: '对字符串处理有特殊技巧，嵌入式开发能力和相关知识提升。',
          color: '#C2185B',
          prob: 0.05,
          kind: 'positive',
          beneficial: true,
        ...
        // 压力转化：压力越高工程设计越强但赛场心态越弱
        this.registerTalent({
          name: '压力转化',
          description: '压力越高，比赛中工程设计越活跃，但赛场心态稳定性越差。',
          color: '#AB47BC',
          prob: 0.04,
          kind: 'positive',
          beneficial: false,
            ...
            try{
              ...
              if(pressure > 50){
                ...
                if(layers > 0 && !student._talent_backup['压力转化']){
                  ...
                  return `压力转化：工程设计+${layers*10}%，赛场心态-${Math.round((1-Math.pow(0.85,layers))*100)}%`;
                }
              }
