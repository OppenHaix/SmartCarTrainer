...
// —— 智能车主题重命名 ——
const SMARTCAR_TASK_PREFIXES = ['[传感调试]', '[动力调校]', '[路径规划]', '[系统协同]', '[算法仿真]', '[赛道测试]', '[软硬整合]', '[安全验证]'];
const SMARTCAR_TASK_SUBJECTS = [
  '磁条弯道', '光照弧段', '雨夜赛段', '桥梁坡道', '地下隧道', '无人机联动', '竞速发车', '激光测距', '紧急避障', '四旋翼协同',
  '虚拟赛道', '多车编队', '高速巡航', '复杂岔路', '传感器阵列', '全局定位', '巡检工位', '动态靶标', '能量补给站', '极寒赛段'
];
const SMARTCAR_TASK_ACTIONS = ['标定', '复测', '调参', '联调', '容错优化', '冗余验证', '线路规划', '策略升级', '参数回放', '赛道记录'];

(function rethemeTaskNames(){
  const used = new Set();
  const prefixLen = SMARTCAR_TASK_PREFIXES.length;
  const subjectLen = SMARTCAR_TASK_SUBJECTS.length;
  const actionLen = SMARTCAR_TASK_ACTIONS.length;

  TASK_POOL.forEach((task, idx) => {
    const prefix = SMARTCAR_TASK_PREFIXES[idx % prefixLen];
    const subject = SMARTCAR_TASK_SUBJECTS[(idx + Math.floor(idx / prefixLen)) % subjectLen];
    const action = SMARTCAR_TASK_ACTIONS[(idx + Math.floor(idx / (prefixLen * 2))) % actionLen];
    let candidate = `${prefix} ${subject}${action}`;
    let attempt = 1;
    while(used.has(candidate)){
      attempt += 1;
      candidate = `${prefix} ${subject}${action}${attempt}`;
    }
    task.legacyName = task.name;
    task.name = candidate;
    used.add(candidate);
  });
})();
...
/**
 * 计算做题增幅的二次函数
 * 当学生能力（工程设计与嵌入式开发平均）等于难度时，增幅 = 1.0（即100%）
 * 能力过高或过低时，增幅都会降低
 * 
 * 使用二次函数: multiplier = 1 - k * (ability - difficulty)^2
 * 当 ability = difficulty 时，multiplier = 1
 * 
 * @param {number} studentAbility - 学生能力（工程设计与嵌入式开发平均值）
