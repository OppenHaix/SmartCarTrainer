...
(function(global){
  ...
  function renderStudentPanels(simulator){
    ...
    const scoreLabel = simulator.isSmartCarRace ? '赛道评分' : '总分';
    const currentLabel = simulator.isSmartCarRace ? '当前元素' : '当前';
    const defaultCurrent = simulator.isSmartCarRace ? '等待发车' : '未选题';
    ...
    for(let i = 0; i < sortedStudents.length; i++){
      ...
      try{
        if(window.SmartCar && s && s.group){
          ...
          groupLabel = (cfg && cfg.display) ? cfg.display : s.group;
        }
      }catch(e){ groupLabel = s && s.group ? s.group : ''; }
      if(!groupLabel){ groupLabel = '未分组'; }

      html += `
        <div class="student-panel" id="student-panel-${s.name}" data-rank="${i + 1}" data-score="${state.totalScore}">
          <div class="student-name">${s.name}</div>
          <div class="student-group">组别: <span>${groupLabel}</span></div>
          <div class="student-score">${scoreLabel}: <span id="score-${s.name}">0</span></div>
          <div class="student-current-problem">
            ${currentLabel}: <span id="current-${s.name}">${defaultCurrent}</span>
          </div>
          <div class="student-problems" id="problems-${s.name}">
            <!-- 题目状态 -->
          </div>
          ${simulator.isSmartCarRace ? `<div style="margin-top:6px"><button class="btn" id="detail-${s.name}">查看元素明细</button></div>` : ''}
        </div>
      `;
    }
    ...
    for(let state of sortedStudents){
      renderProblemStatus(state, simulator);
      if(simulator.isSmartCarRace){
        const btn = document.getElementById(`detail-${state.student.name}`);
        if(btn){
          btn.addEventListener('click', () => {
            try{ showElementDetailModal(state, simulator); }catch(e){ console.error('showElementDetailModal error', e); }
          });
        }
      }
    }
   ...
   * 渲染单个学生的题目状态
   */
  function renderProblemStatus(state, simulator){
    ...
    for(let prob of state.problems){
      ...
      const label = simulator.isSmartCarRace && prob.trackElement ? escapeHtml(prob.trackElement.name) : `T${prob.id + 1}`;
      html += `
        <div class="problem-item ${statusClass}" id="prob-${state.student.name}-${prob.id}">
          <div class="problem-id">${label}</div>
          <div class="problem-score">${Math.round(prob.maxScore || 0)}</div>
        </div>
      `;
    }
    html += '</div>';
  ...
  function updateContestProgress(tick, maxTicks, simulator){
    ...
    if(currentTimeEl){
      if(simulator.isSmartCarRace){
        const intervalSec = simulator.tickIntervalSeconds || (simulator.tickIntervalMinutes ? simulator.tickIntervalMinutes * 60 : (typeof simulator.constructor.TICK_INTERVAL !== 'undefined' ? simulator.constructor.TICK_INTERVAL * 60 : 600));
        const seconds = Number(tick) * intervalSec;
        currentTimeEl.textContent = formatSeconds(seconds);
      } else {
        const interval = simulator.tickIntervalMinutes || (typeof simulator.constructor.TICK_INTERVAL !== 'undefined' ? simulator.constructor.TICK_INTERVAL : 10);
    ...
    if(progressBar){
      ...
      let progress = (safeTick / safeMax) * 100;
      if(simulator.isSmartCarRace){
        const intervalSec = simulator.tickIntervalSeconds || (simulator.tickIntervalMinutes ? simulator.tickIntervalMinutes * 60 : 30);
        const elapsed = Number(tick) * intervalSec;
        const total = safeMax * intervalSec;
        progress = total > 0 ? (Math.min(elapsed, total) / total) * 100 : progress;
      }
  ...
  function updateStudentPanels(students, simulator){
    ...
    for(const state of students){
      ...
      if(currentEl){
        if(simulator.isSmartCarRace){
          ...
          if(track && track.completed){
            const finalTime = track.finalTimeSec || (track.totalTimeSec + track.penaltySec);
            const penaltyPart = track.penaltySec > 0 ? ` + 罚时${formatSeconds(track.penaltySec)}` : '';
            currentEl.textContent = `完成 (${formatSeconds(finalTime)}${penaltyPart})`;
          } else if(state.currentTarget !== null){
            const prob = state.getProblem(state.currentTarget);
            const elementName = prob && prob.trackElement ? prob.trackElement.name : `元素${state.currentTarget + 1}`;
            const runtime = prob && prob.trackRuntime ? prob.trackRuntime.timeSec || 0 : 0;
            const penalties = prob && prob.trackRuntime ? prob.trackRuntime.penaltySec || 0 : 0;
            const detail = penalties > 0 ? `${formatSeconds(runtime)} + 罚时${formatSeconds(penalties)}` : `${formatSeconds(runtime)}`;
            currentEl.textContent = `${elementName} (${detail})`;
          } else {
            currentEl.textContent = '等待发车';
          }
        } else {
      ...
      for(const prob of state.problems){
        ...
        if(scoreSpan) scoreSpan.textContent = String(Math.round(prob.maxScore || 0));
      }
   ...
   * 显示智能车赛道元素明细弹窗
   */
  function showElementDetailModal(state, simulator){
    const modalRoot = document.getElementById('modal-root');
    if(!modalRoot) return;
    const rows = [];
    const track = state.trackState || {};
    for(const prob of state.problems){
      ...
      const rt = prob.trackRuntime || { attempts: 0, timeSec: 0, penaltySec: 0 };
      const name = el ? el.name : `元素${(prob.id||0)+1}`;
      const status = track.completed ? (prob.solved ? '通过' : (rt.penaltySec>0?'受罚':'未通过')) : (prob.solved ? '通过' : '进行中');
      rows.push({
        name,
        attempts: rt.attempts || 0,
        timeSec: rt.timeSec || 0,
        penaltySec: rt.penaltySec || 0,
        score: Math.round(prob.scoreAchieved || prob.maxScore || 0),
        status
      });
    }
    const tableRows = rows.map(r => `
      <tr>
        <td style="padding:6px;border-bottom:1px solid #eee">${escapeHtml(r.name)}</td>
        <td style="padding:6px;text-align:center;border-bottom:1px solid #eee">${r.attempts}</td>
        <td style="padding:6px;text-align:center;border-bottom:1px solid #eee">${formatSeconds(r.timeSec)}</td>
        <td style="padding:6px;text-align:center;border-bottom:1px solid #eee">${r.penaltySec>0?formatSeconds(r.penaltySec):'—'}</td>
        <td style="padding:6px;text-align:center;border-bottom:1px solid #eee">${r.score}</td>
        <td style="padding:6px;text-align:center;border-bottom:1px solid #eee">${r.status}</td>
      </tr>
    `).join('');
    const finalTime = track && typeof track.finalTimeSec === 'number' ? track.finalTimeSec : ((track.totalTimeSec||0)+(track.penaltySec||0));
    const penaltyPart = (track.penaltySec||0) > 0 ? `（罚时${formatSeconds(track.penaltySec||0)}）` : '';
    const html = `
      <div class="modal" style="z-index:2100">
        <div class="dialog" style="max-width:800px">
          <h3 style="margin-bottom:8px">${escapeHtml(state.student.name)} 的赛道元素明细</h3>
          <div class="small muted" style="margin-bottom:10px">总用时：${formatSeconds(finalTime)} ${penaltyPart}</div>
          <div style="max-height:420px;overflow:auto;border:1px solid #e5e7eb;border-radius:6px">
            <table style="width:100%;border-collapse:collapse;font-size:13px">
              <thead>
                <tr style="background:#f9fafb">
                  <th style="padding:6px;text-align:left">元素</th>
                  <th style="padding:6px;text-align:center">尝试</th>
                  <th style="padding:6px;text-align:center">用时</th>
                  <th style="padding:6px;text-align:center">罚时</th>
                  <th style="padding:6px;text-align:center">得分</th>
    ...
    wrapper.id = 'contest-submodal';
    wrapper.innerHTML = html;
    modalRoot.appendChild(wrapper);
    const closeBtn = wrapper.querySelector('#detail-close');
    if(closeBtn){ closeBtn.onclick = () => { try{ wrapper.remove(); }catch(e){} }; }
  }
