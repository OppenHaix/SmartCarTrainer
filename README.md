
# SmartCarTrainer — 智能车教练模拟器

这是一个基于浏览器的文字/策略模拟游戏，模拟智能车教练训练选手从赛季训练到正式比赛的成长与抉择。  
项目由 **HTML / JavaScript / CSS** 构建，无需后端即可运行。

## 注：原作改版信息
本项目基于OItrainer — 信息学教练模拟器进行改版
原作链接为[OItrainer](https://github.com/seve42/OItrainer)

---

## 🏫 游戏特色

- **赛季推进与赛事模拟**：完整的赛季与周推进系统，覆盖校内选拔赛、区域赛、全国总决赛等关键节点。
- **细化的队员模型**：包含工程设计、嵌入式开发、赛场心态与五大核心模块，并考虑压力、舒适度、天气、生病等状态。
- **多样训练与外出集训**：提供多种选项提升选手能力。  
- **天赋与事件系统**：增强趣味性与策略深度。  
- **设施与预算管理**：设施经费、集训经费等限制提升决策复杂度。  
- **模拟赛与比赛评分**：根据时间节拍、子任务与知识门槛还原比赛过程，实时滚榜，并触发天赋与技能效果。  

详细机制与玩法建议请参阅项目内的 `help.md` 或游戏内帮助。

---

## 🎮 游玩方式

直接访问 👉 [[https://trainer.starxi.cn](https://trainer.starxi.cn/)] 即可开始游戏。

---

## 🧩 开发信息

- **项目类型**：前端静态页面（HTML + JavaScript + CSS）  
- **主要目录结构**：
  - `index.html`：主入口  
  - `game.js`, `render.js`, `debug.js`, `lib/`：核心逻辑脚本（比赛模拟、天赋、事件、模型等）  
  - `assets/`：图片与资源文件  
  - 其他脚本如 `tutorial.js`：用于教学与新手引导  

若需修改并本地调试，建议使用带开发者工具的浏览器（Chrome / Edge / Firefox）。  

---

## 🤝 贡献与反馈

欢迎提交 Issue 或 Pull Request 来改进游戏体验。

---

## ⚖️ 许可协议

本项目采用 **Apache License 2.0**：

> 欢迎在保留原作者信息与许可声明的前提下进行魔改或微调。  
> 如果你制作了改版，请在开始页面 **明显标注原版链接与改版说明**，  
> 并在网站中说明主要修改内容，以符合 Apache License 2.0 协议。

此外，本游戏的数值均经过精心设计与联动平衡。  
若计划增添模块或修改参数，请谨慎避免数值崩坏。

---

## ✉️ 致谢与联系方式

原版OItrainer
- 作者：**seve42**  
- Luogu：`seve_`  
- 邮箱：`dreamer-seve@outlook.com` 或 `dreamersseve@gmail.com`

再版SmartCarTrainer
- 作者：**奥本海星**
- 邮箱：`zyscr@outlook.com`
---

##  为什么不提供自定义学生姓名？

这个功能实现非常简单，几行代码几分钟即可完成，也被问过很多次。  
但我始终没有加入它。

我希望玩家面对的，不是“自己塑造的角色”，而是**一个客观存在的学生**。  
你不是在写同人，也不是在养自己的人设，而是在做教练、做决策、观察成长与失败。

若允许自定义姓名，游戏会立刻变成个人化的舞台——  
人们会把朋友、偶像、自己或虚构人物放进去，沉迷于二次创作、恶搞甚至政治隐喻，  
而不再思考“一个教练该如何训练、如何面对有限的资源与时间”。

当然，找乐子也无妨。  
你仍然可以按下 F12，自行输入：

```

game.students.at(<编号>).name = "seve42"

```

这是你的自由选择。
