...
class Student {
  ...
  getKnowledgeByType(type){
    if(type==='数据结构' || type==='硬件设计') return this.knowledge_ds;
    if(type==='图论' || type==='传感与识别') return this.knowledge_graph;
    if(type==='字符串' || type==='运动规划') return this.knowledge_string;
    if(type==='数学' || type==='系统建模') return this.knowledge_math;
    if(type==='DP' || type==='动态规划' || type==='算法优化') return this.knowledge_dp;
    return 0;
  }
  addKnowledge(type,amount){
    ...
    if(type==='数据结构' || type==='硬件设计') this.knowledge_ds += safeAmount;
    else if(type==='图论' || type==='传感与识别') this.knowledge_graph += safeAmount;
    else if(type==='字符串' || type==='运动规划') this.knowledge_string += safeAmount;
    else if(type==='数学' || type==='系统建模') this.knowledge_math += safeAmount;
    else if(type==='DP' || type==='动态规划' || type==='算法优化') this.knowledge_dp += safeAmount;
  }
  // 统一接口：为思维/代码提供受控增幅方法
...
const SMARTCAR_ATTRIBUTE_ALIASES = {
  hardwareDesign: 'knowledge_ds',
  sensorRecognition: 'knowledge_graph',
  motionPlanning: 'knowledge_string',
  systemModeling: 'knowledge_math',
  algorithmOptimization: 'knowledge_dp'
};

for(const [alias, baseProp] of Object.entries(SMARTCAR_ATTRIBUTE_ALIASES)){
  Object.defineProperty(Student.prototype, alias, {
    get(){ return Number(this[baseProp] || 0); },
    set(v){ this[baseProp] = Number(v); },
    enumerable: true,
    configurable: true
  });
}
