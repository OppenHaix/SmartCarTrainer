...
const COMPETITION_SCHEDULE = [
  ...
  {week:9, name:"校内选拔赛", difficulty:65, maxScore:100, numProblems:1, smartCarRace:true},
  {week:17, name:"技术方案评审", difficulty:145, maxScore:400, numProblems:4, smartCarRace:true},
  {week:25, name:"区域赛", difficulty:205, maxScore:400, numProblems:4, smartCarRace:true},
  {week:40, name:"分赛区决赛", difficulty:340, maxScore:600, numProblems:6, smartCarRace:true},
  {week:52, name:"全国总决赛", difficulty:440, maxScore:700, numProblems:7, smartCarRace:true},
  // 国家集训队比赛（仅在第二年全国总决赛金奖且接受后才会生效）
  // 这些比赛会在接受国家集训队后动态添加到competitions数组，这里的week仅作参考
  // 实际week计算：全国总决赛结束后 + 2周（集训营-硬件冲刺）、+3周（集训营-算法对抗）、+4周（国家队集训考核）、考核后+1周（世界智能车锦标赛）
  {week:54, name:"集训营-硬件冲刺", difficulty:520, maxScore:600, numProblems:6, nationalTeam:true, smartCarRace:true},
  {week:55, name:"集训营-算法对抗", difficulty:520, maxScore:600, numProblems:6, nationalTeam:true, smartCarRace:true},
  {week:56, name:"国家队集训考核", difficulty:550, maxScore:800, numProblems:8, nationalTeam:true, smartCarRace:true},
  {week:57, name:"世界智能车锦标赛", difficulty:650, maxScore:600, numProblems:6, nationalTeam:true, smartCarRace:true, subtasksPerProblem:15}
];
...
const COMPETITION_DIFFICULTY_FACTORS = {
  "校内选拔赛": [1.0],  // 单题，无需多个系数
  "技术方案评审": [0.5, 1.0, 1.5, 2.0],
  "区域赛": [0.5, 0.8, 1.5, 2.5],
  "分赛区决赛": [0.7, 0.7, 1.2, 1.5, 1.8, 1.8],
  "全国总决赛": [0.5, 0.8, 1.0, 1.2, 1.3, 1.3, 1.5],
  // 国家集训队比赛难度系数
  "集训营-硬件冲刺": [1.2, 1.2, 1.2, 1.5, 1.5, 1.5],
  "集训营-算法对抗": [1.2, 1.2, 1.2, 1.5, 1.5, 1.5],
  "国家队集训考核": [1.0, 1.2, 1.2, 1.5, 1.5, 1.7, 1.7, 1.7],
  "世界智能车锦标赛": [0.8, 0.8, 1.2, 1.5, 1.8, 2.2]
};

// 明确的比赛链顺序（用于链式晋级判断）
// 国家集训队比赛不在此列表中，因为它们的晋级逻辑是特殊的
const COMPETITION_ORDER = ["校内选拔赛","技术方案评审","区域赛","分赛区决赛","全国总决赛"];
...
const CONTEST_GAIN_RATIOS = {
  // 正式比赛
  '校内选拔赛': { knowledge: 0.3, thinking: 0.3, coding: 0.3 },      // 入门级
  '技术方案评审': { knowledge: 0.5, thinking: 0.5, coding: 0.5 },      // 普及级
  '区域赛': { knowledge: 0.7, thinking: 0.7, coding: 0.7 },        // 区域赛级
  '分赛区决赛': { knowledge: 0.9, thinking: 0.9, coding: 0.9 },        // 分赛区决赛级
  '全国总决赛': { knowledge: 1.0, thinking: 1.0, coding: 1.0 },         // 全国总决赛级（最高）
...
const NOI_REWARD_MIN = 30000;  // 全国总决赛奖励（保持变量名兼容）
const NOI_REWARD_MAX = 50000;
const NOIP_REWARD_MIN = 10000; // 区域赛奖励（保持变量名兼容）
