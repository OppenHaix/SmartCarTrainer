/* competitions.js - 比赛模拟引擎
   ...
   比赛时长：
   - 校内选拔赛: 120分钟 (12个tick)
   - 技术方案评审/区域赛: 240分钟 (24个tick)
   - 分赛区决赛/全国总决赛: 480分钟 (48个tick)
*/
...
(function(global){
  ...
  function generateSubtasks(totalScore, problemDifficulty, thinkingBase=null, codingBase=null, options={}){
    ...
    // 支持指定subtask数量（例如世界智能车锦标赛的15个测试点）
  ...
  class ContestSimulator {
    constructor(contestConfig, students, game){
      ...
      this.game = game;
      this.isSmartCarRace = Boolean(contestConfig && contestConfig.isSmartCarRace);
      this.tickIntervalMinutes = this.isSmartCarRace
        ? (contestConfig.tickIntervalMinutes || 0.5)
        : TICK_INTERVAL;
      this.tickIntervalSeconds = this.tickIntervalMinutes * 60;
      this.students = students.map(s => new StudentContestState(s, contestConfig.problems));
      this.currentTick = 0;
      const durationMinutes = contestConfig && contestConfig.duration ? contestConfig.duration : 60;
      this.maxTicks = Math.max(1, Math.ceil(durationMinutes / this.tickIntervalMinutes));
      ...
      if(this.isSmartCarRace){
        this.initializeSmartCarStates();
      }
    }

    initializeSmartCarStates(){
      const bundle = this.config.trackBundle || {};
      const trackByGroup = this.config.trackByGroup || bundle.trackByGroup || {};
      const defaultGroup = (global.SmartCar && typeof global.SmartCar.getDefaultGroup === 'function')
        ? global.SmartCar.getDefaultGroup()
        : (Object.keys(trackByGroup)[0] || null);

      for(const state of this.students){
        const student = state.student;
        try{
          if(global.SmartCar && typeof global.SmartCar.ensureGroup === 'function'){
            global.SmartCar.ensureGroup(student);
          }
        }catch(e){ /* ignore */ }

        const groupName = (student.group && trackByGroup[student.group]) ? student.group : defaultGroup;
        const template = trackByGroup[groupName] || trackByGroup[defaultGroup] || [];

        state.trackState = {
          group: groupName,
          elements: template.map(el => Object.assign({}, el)),
          currentIndex: 0,
          totalTimeSec: 0,
          penaltySec: 0,
          finalTimeSec: template.length === 0 ? 0 : null,
          completed: template.length === 0,
          disqualified: false
        };

        const limit = Math.min(state.problems.length, state.trackState.elements.length);
        for(let i = 0; i < limit; i++){
          const prob = state.problems[i];
          const element = state.trackState.elements[i];
          if(element){
            prob.trackElement = Object.assign({}, element);
            prob.targetScore = element.targetScore;
            prob.scoreWeight = element.scoreWeight || prob.scoreWeight || 1;
            if(!prob.subtasks || prob.subtasks.length === 0){
              const difficultyValue = Math.max(1, Math.round(element.difficulty || this.config.originalDef?.difficulty || 60));
              const scoreValue = Math.max(1, Math.round(element.targetScore || 0));
              prob.subtasks = [{
                score: scoreValue,
                difficulty: difficultyValue,
                thinkingDifficulty: difficultyValue,
                codingDifficulty: difficultyValue
              }];
            }
          } else {
            prob.trackElement = null;
          }
          prob.scoreAchieved = 0;
        }
        if(state.problems.length !== limit){
          state.problems = state.problems.slice(0, limit);
        }

        state.currentTarget = state.trackState.completed ? null : 0;
      }
    }
    ...
    addLog(message, type = 'info', studentName = null){
      try{
        const lvl = (typeof global.LOG_LEVEL !== 'undefined') ? String(global.LOG_LEVEL || 'warn') : 'warn';
        const verbose = (typeof global.ENABLE_VERBOSE_LOG !== 'undefined') ? Boolean(global.ENABLE_VERBOSE_LOG) : false;
        const levelRank = { debug: 0, info: 1, warn: 2, error: 3 };
        const typeRank = { info: 1, select: 1, skip: 2, talent: 2, solve: 2 };
        const shouldEmit = verbose || (typeRank[type] || 1) >= (levelRank[lvl] || 2);
        if(!shouldEmit) return;
      }catch(e){ /* ignore gating errors */ }
      const log = {
        tick: this.currentTick,
        time: this.currentTick * this.tickIntervalMinutes, // 比赛时间（分钟）
        message: message,
    ...
    refreshSmartCarScore(state){
      if(!state || !state.problems) return;
      let sum = 0;
      for(const prob of state.problems){
        if(typeof prob.scoreAchieved === 'number' && isFinite(prob.scoreAchieved)){
          sum += prob.scoreAchieved;
        }
      }
      state.totalScore = Math.max(0, Math.round(sum));
    }

    simulateSmartCarTick(state){
      ...
      const student = state.student;
      const currentIndex = Math.max(0, track.currentIndex);
      const prob = state.getProblem(currentIndex);
      if(!prob || !prob.trackElement){
        track.completed = true;
        state.currentTarget = null;
        return;
      }

      if(!prob.trackRuntime){
        prob.trackRuntime = { attempts: 0, timeSec: 0, penaltySec: 0 };
      }

      state.currentTarget = currentIndex;
      prob.trackRuntime.attempts++;

      let evaluation;
      try{
        if(global.SmartCar && typeof global.SmartCar.evaluateElement === 'function'){
          evaluation = global.SmartCar.evaluateElement(student, prob.trackElement, { contest: this.config });
        }
      }catch(e){ evaluation = null; }

      if(!evaluation){
        evaluation = {
          abilityScore: typeof student.getComprehensiveAbility === 'function' ? student.getComprehensiveAbility() : 60,
          successRate: 0.5,
          mental: typeof student.getMentalIndex === 'function' ? student.getMentalIndex() : (student.mental || 50)
        };
      }

      let successRate = typeof evaluation.successRate === 'number' ? evaluation.successRate : 0.5;
      successRate = Math.max(0.08, Math.min(0.98, successRate));

      const element = prob.trackElement;

      if(Math.random() <= successRate){
        let timeSec;
        try{
          if(global.SmartCar && typeof global.SmartCar.computeElementTime === 'function'){
            timeSec = global.SmartCar.computeElementTime(student, element, evaluation, { contest: this.config });
          }
        }catch(e){ timeSec = null; }
        if(!timeSec || !isFinite(timeSec)){
          timeSec = element.baseTimeSec;
        }

        prob.trackRuntime.timeSec = (prob.trackRuntime.timeSec || 0) + timeSec;
        track.totalTimeSec += timeSec;

        const runtimeInfo = { timeSec: prob.trackRuntime.timeSec, penaltySec: prob.trackRuntime.penaltySec || 0 };

        let elementScore;
        try{
          if(global.SmartCar && typeof global.SmartCar.computeElementScore === 'function'){
            elementScore = global.SmartCar.computeElementScore(element, runtimeInfo, { contest: this.config });
          }
        }catch(e){ elementScore = null; }
        if(!elementScore || !isFinite(elementScore)){
          const perSecond = this.config.scorePerSecond || 0.2;
          elementScore = Math.max(0, (element.targetScore || 0) - (runtimeInfo.timeSec + runtimeInfo.penaltySec * 0.9) * perSecond);
        }

        const roundedScore = Math.max(1, Math.round(elementScore));
        prob.scoreAchieved = Math.max(1, elementScore);
        prob.maxScore = roundedScore;
        prob.solved = true;

        this.refreshSmartCarScore(state);

        const accPenalty = prob.trackRuntime.penaltySec || 0;
        this.addLog(`${student.name} 成功通过【${element.name}】，本元素得分${roundedScore}，用时${formatSeconds(timeSec)}，累计罚时${formatSeconds(accPenalty)}`,'solve', student.name);

        track.currentIndex++;
        if(track.currentIndex >= track.elements.length){
          track.completed = true;
          track.finalTimeSec = track.totalTimeSec + track.penaltySec;
          state.currentTarget = null;
          const penaltyPart = track.penaltySec > 0 ? `（罚时${formatSeconds(track.penaltySec)}）` : '';
          this.addLog(`${student.name} 完成全赛道，总用时${formatSeconds(track.finalTimeSec)}${penaltyPart}`, 'info', student.name);
        }
        state.thinkingTime = 0;
      } else {
        let penaltySec;
        try{
          if(global.SmartCar && typeof global.SmartCar.computeElementPenalty === 'function'){
            penaltySec = global.SmartCar.computeElementPenalty(student, element, evaluation, { contest: this.config });
          }
        }catch(e){ penaltySec = null; }
        if(!penaltySec || !isFinite(penaltySec)){
          penaltySec = element.penaltyTimeSec;
        }

        // 失败尝试仍然消耗一定基础时间
        const wastedTime = Math.max(3, element.baseTimeSec * 0.6);
        prob.trackRuntime.timeSec = (prob.trackRuntime.timeSec || 0) + wastedTime;
        track.totalTimeSec += wastedTime;

        prob.trackRuntime.penaltySec = (prob.trackRuntime.penaltySec || 0) + penaltySec;
        track.penaltySec += penaltySec;

        let penaltyLoss;
        try{
          if(global.SmartCar && typeof global.SmartCar.computePenaltyScoreLoss === 'function'){
            penaltyLoss = global.SmartCar.computePenaltyScoreLoss(element, penaltySec, { contest: this.config, runtime: prob.trackRuntime });
          }
        }catch(e){ penaltyLoss = null; }
        if(!penaltyLoss || !isFinite(penaltyLoss)){
          penaltyLoss = penaltySec * (this.config.scorePerSecond || 0.2);
        }

        const prevScore = prob.scoreAchieved || 0;
        const newScore = Math.max(0, prevScore - penaltyLoss);
        prob.scoreAchieved = newScore;
        prob.maxScore = Math.round(newScore);
        prob.solved = false;
        ...
        let reason;
        try{
          if(global.SmartCar && typeof global.SmartCar.pickPenaltyReason === 'function'){
            reason = global.SmartCar.pickPenaltyReason(element, { contest: this.config, runtime: prob.trackRuntime });
          }
        }catch(e){ reason = null; }
        if(!reason) reason = '执行出现误差';
        this.addLog(`${student.name} 未通过【${element.name}】，${reason}，罚时${formatSeconds(penaltySec)}，重新尝试`, 'info', student.name);

        try{
          if(global.SmartCar && typeof global.SmartCar.checkDisqualification === 'function' && global.SmartCar.checkDisqualification(element, { contest: this.config, runtime: prob.trackRuntime })){
            track.disqualified = true;
            track.completed = true;
            track.finalTimeSec = Infinity;
            state.totalScore = 0;
            state.currentTarget = null;
            this.addLog(`${student.name} 因重大违规被取消成绩`, 'talent', student.name);
          }
        }catch(e){ /* ignore */ }
      }

      if(prob.trackRuntime){
        state.thinkingTime = prob.trackRuntime.timeSec || 0;
      }
    ...
    runTick(){
      ...
      if(this.isSmartCarRace){
        const allDone = this.students.every(st => !st.trackState || st.trackState.completed);
        if(allDone){
          this.finish();
          return;
        }
    ...
    simulateStudentTick(state){
      if(this.isSmartCarRace){
        this.simulateSmartCarTick(state);
        return;
      }
      ...
      state.thinkingTime += this.tickIntervalMinutes;
    ...
    finish(){
      ...
      if(this.isSmartCarRace){
        for(const st of this.students){
          const track = st.trackState;
          if(track){
            if(typeof track.finalTimeSec !== 'number' || !isFinite(track.finalTimeSec)){
              track.finalTimeSec = track.totalTimeSec + track.penaltySec;
            }
            track.completed = true;
          }
      ...
      // ========== 失误系统 ==========
      // 仅在区域赛、技术方案评审、分赛区决赛比赛中生效
      const mistakeApplicableContests = ['区域赛', '技术方案评审', '分赛区决赛'];
      if(!this.isSmartCarRace && mistakeApplicableContests.includes(this.config.name)){
    ...
    // 获取剩余时间（分钟）
    getRemainingTime(){
      return (this.maxTicks - this.currentTick) * this.tickIntervalMinutes;
    }
  }
  ...
  function buildContestConfig(contestDef){
    if(contestDef && contestDef.smartCarRace && global.SmartCar && typeof global.SmartCar.prepareContestTracks === 'function'){
      try{
        const trackBundle = global.SmartCar.prepareContestTracks(contestDef);
        if(trackBundle && trackBundle.templateTrack && trackBundle.templateTrack.length){
          const effectiveTrack = trackBundle.templateTrack.slice(0, Math.max(1, trackBundle.minTrackLength || trackBundle.templateTrack.length));
          const problems = effectiveTrack.map((element, idx) => {
            const difficultyValue = Math.max(1, Math.round(element.difficulty || contestDef.difficulty || 60));
            const scoreValue = Math.max(1, Math.round(element.targetScore || ((contestDef.maxScore || 100) / Math.max(1, trackBundle.trackLength || trackBundle.templateTrack.length))));
            return {
              id: idx,
              tags: [element.type || '赛道元素'],
              subtasks: [{
                score: scoreValue,
                difficulty: difficultyValue,
                thinkingDifficulty: difficultyValue,
              ...
              trackElementTemplate: Object.assign({}, element),
          ...
          return {
            ...
            duration: Math.max(4, trackBundle.durationMinutes || contestDef.duration || 6),
            problems,
            originalDef: contestDef,
            isSmartCarRace: true,
            trackBundle,
            trackByGroup: trackBundle.trackByGroup,
            trackLength: Math.max(1, trackBundle.minTrackLength || trackBundle.trackLength),
            trackEstimatedMaxTimeSec: trackBundle.estimatedMaxTimeSec,
            tickIntervalMinutes: trackBundle.tickIntervalMinutes || 0.5,
            scorePerSecond: trackBundle.scorePerSecond
          };
        }
      }catch(e){ console.error('prepareContestTracks error', e); }
    }
