/* national-team.js - 国家集训队相关逻辑
   包含国家集训队选择、集训营/国家队考核、世界智能车锦标赛选拔等功能
*/
...
(function(global){
  ...
  /**
   * 显示国家集训队选择弹窗
   * @param {Array} noiResults - 全国总决赛结果
   * @param {number} noiMaxScore - 全国总决赛最高分
  ...
  function showNationalTeamChoice(noiResults, noiMaxScore, passLine) {
    ...
    html += `<div style="margin-bottom:8px;">以下队员在全国总决赛中获得金奖，拿到国家集训队资格：</div>`;
    ...
    html += `<p>• 进入国家集训队后，将进行集训营冲刺与国家队集训考核</p>`;
    html += `<p>• 没有金牌的学生将自动退队（不减少声誉）</p>`;
    html += `<p>• 根据集训营与考核总分排名，入选国家队，参加世界智能车锦标赛</p>`;
    html += `</div>`;
    html += `</div>`;
  ...
  function enterNationalTeam(goldStudents) {
    ...
    const nationalTeamComps = [
      {week: currentWeek + 2, name:"集训营-硬件冲刺", difficulty:500, maxScore:600, numProblems:6, nationalTeam:true},
      {week: currentWeek + 3, name:"集训营-算法对抗", difficulty:500, maxScore:600, numProblems:6, nationalTeam:true},
      {week: currentWeek + 4, name:"国家队集训考核", difficulty:520, maxScore:800, numProblems:8, nationalTeam:true}
      // 世界智能车锦标赛会在国家队集训考核结束后根据晋级情况动态添加
    ];
  ...
  function calculateNationalTeamQualification() {
    ...
    for(let i = 0; i < totalScores.length; i++){
      ...
      const resultText = isQualified ? '✅ 晋级世界智能车锦标赛' : '❌ 未晋级';
    ...
    if(qualified.length > 0){
      html += `<div style="background:#fff9c4;padding:12px;border-radius:5px;margin-bottom:12px;border:2px solid #fbc02d;">`;
      html += `<div style="font-size:16px;font-weight:bold;margin-bottom:8px;color:#f57f17;">🎉 恭喜晋级世界智能车锦标赛！</div>`;
      html += `<div>晋级队员：${qualified.map(q => q.name).join('、')}</div>`;
      html += `<div style="margin-top:8px;font-size:13px;">1周后将参加世界智能车锦标赛</div>`;
      html += `</div>`;
    } else {
    ...
    if(confirmBtn){
      confirmBtn.onclick = ()=>{
        ...
        if(qualified.length > 0){
          // 有人晋级，动态添加世界智能车锦标赛到competitions数组
          const currentWeek = game.week;
          const ioiComp = {
            week: currentWeek + 1,
            name: "世界智能车锦标赛",
          ...
          // 检查是否已添加世界智能车锦标赛
          const hasIOI = global.competitions && global.competitions.some(c => c.name === '世界智能车锦标赛' && c.nationalTeam);
          if(!hasIOI && global.competitions){
            global.competitions.push(ioiComp);
            console.log('【国家集训队】动态添加世界智能车锦标赛到competitions:', ioiComp);
          }
          
          if(typeof global.log === 'function'){
            global.log(`世界智能车锦标赛将在第 ${ioiComp.week} 周进行`);
          }
          
          if(typeof global.pushEvent === 'function'){
            global.pushEvent({
              name: '世界智能车锦标赛晋级',
              description: `${qualified.length}名队员晋级世界智能车锦标赛`,
              week: game.week
            });
          }
  ...
  function calculateIOIResults() {
    ...
    // 从 careerCompetitions 中提取世界智能车锦标赛成绩
    const ioiRecord = game.careerCompetitions.find(c => c.name === '世界智能车锦标赛');
    if(!ioiRecord){
      console.error('未找到世界智能车锦标赛比赛记录');
      return;
    }
    
    // 世界智能车锦标赛奖牌分配：不再使用固定的基础分数线（IOI_GOLD_THRESHOLD 等），
    // 仅按当前参赛人数的排名百分比分配奖牌（默认 10%/30%/50%）。
    const maxScore = 600; // 世界智能车锦标赛满分（仅用于检测满分情形）
    ...
    if(chineseHasFullScore){
      ...
      html += `<div>恭喜！中国队选手在世界智能车锦标赛上取得满分，这是最高荣誉！</div>`;
      html += `</div>`;
    } else if(chineseGoldCount > 0){
      html += `<div style="background:#fff9c4;padding:12px;border-radius:5px;margin-bottom:12px;border:2px solid #fbc02d;">`;
      html += `<div style="font-size:16px;font-weight:bold;margin-bottom:8px;color:#f57f17;">🥇 获得世界智能车锦标赛金牌！</div>`;
      html += `<div>恭喜！中国队选手获得${chineseGoldCount}枚金牌，达成顶尖结局！</div>`;
      html += `</div>`;
    } else if(chineseMedalCount > 0){
      html += `<div style="background:#e3f2fd;padding:12px;border-radius:5px;margin-bottom:12px;">`;
      html += `<div style="font-size:16px;font-weight:bold;margin-bottom:8px;color:#1976d2;">🏅 获得世界智能车锦标赛奖牌！</div>`;
      ...
      html += `<div>虽然中国队未获得奖牌，但选手参加世界智能车锦标赛本身就是了不起的成就！</div>`;
      html += `</div>`;
    }
