...
(function(global){
  ...
  function logAt(level){
    ...
    const curr = (global.LOG_LEVEL || 'warn');
    const args = Array.prototype.slice.call(arguments, 1);
    if(level === 'debug' && !global.ENABLE_VERBOSE_LOG){
      if(order[curr] >= 3){ try{ console.log.apply(console, args); }catch(e){} }
      return;
    }
    if(order[level] <= order[curr]){
      try{
  ...
  function formatSeconds(sec){
    if(!isFinite(sec) || sec <= 0) return '0秒';
    if(sec >= 120) return `${(sec/60).toFixed(1)}分钟`;
    if(sec >= 60) return `${(sec/60).toFixed(2)}分钟`;
  ...
  for(const t of talents) student.talents.add(t);
    ...
    logAt('info', `[IOI国际赛] 生成了 ${internationalStudents.length} 名国际选手，覆盖国家数: ${countries.length-1}`);
  ...
  const halfIndex = (game.week > halfBoundary) ? 1 : 0;
    ...
    // IOI 特殊标记（后续资格检查中使用）
    const isIOI = comp.name === '世界智能车锦标赛' && comp.nationalTeam;
    
    // 筛选活跃学生并检查晋级资格
    let eligibleStudents = [];
    let ineligibleStudents = [];
    
    logAt('info', `[比赛开始] ${comp.name}, 赛季 ${halfIndex}, 第 ${game.week} 周`);
    
    // 显示当前赛季的晋级资格状态
    if(game.qualification && game.qualification[halfIndex]){
      logAt('debug', `[当前赛季晋级状态]`, game.qualification[halfIndex]);
    } else {
      logAt('debug', `[当前赛季晋级状态] 无晋级记录`);
    }
    ...
    for(let s of game.students){
      ...
      if(isIOI){
        ...
        if(!isInIOIList){
          isEligible = false;
          logAt('info', `[IOI资格检查] ❌ ${s.name} 未晋级IOI，无法参加 ${comp.name}`);
        } else {
          logAt('info', `[IOI资格检查] ✅ ${s.name} 已晋级IOI，可以参加 ${comp.name}`);
        }
      } 
      // 如果不是第一场比赛（且不是IOI），需要检查上一场比赛的晋级资格
        ...
        if(!hasQualification){
          isEligible = false;
          logAt('info', `[晋级检查] ❌ ${s.name} 未晋级 ${prevComp}，无法参加 ${comp.name}`);
        } else {
          logAt('info', `[晋级检查] ✅ ${s.name} 已晋级 ${prevComp}，可以参加 ${comp.name}`);
        }
      } else {
      logAt('info', `[晋级检查] ✅ ${s.name} 可以参加 ${comp.name}（首场比赛）`);
      }
    ...
    logAt('info', `[参赛统计] ${comp.name}: 有资格 ${eligibleStudents.length} 人, 无资格 ${ineligibleStudents.length} 人`);
    
    // IOI 特殊处理：添加国际选手
    let internationalStudents = [];
    if(comp.name === '世界智能车锦标赛' && comp.nationalTeam){
      // 调试日志：显示 IOI 晋级资格信息
      if(game.nationalTeamResults && game.nationalTeamResults.ioiQualified){
        logAt('debug', `[IOI资格调试] ioiQualified列表:`, game.nationalTeamResults.ioiQualified);
      } else {
        logAt('debug', `[IOI资格调试] 未找到ioiQualified列表`);
      }
      logAt('debug', `[IOI资格调试] 参赛学生:`, eligibleStudents.map(s => s.name));
      logAt('debug', `[IOI资格调试] 未参赛学生:`, ineligibleStudents.map(s => s.name));
      
      internationalStudents = generateInternationalStudents(eligibleStudents);
      logAt('info', `[IOI国际赛] 生成了 ${internationalStudents.length} 名国际选手`);
    ...
    if(eligibleStudents.length === 0){
      ...
      logAt('debug', '【DEBUG】 skip competition in season 2, triggering ending, careerCompetitions:', game.careerCompetitions);
      ...
      logAt('debug', '【DEBUG】 skip competition, careerCompetitions after push:', game.careerCompetitions);
  ...
  function handleCompetitionResults(studentStates, config, originalComp, ineligibleStudents, internationalStudents){
    ...
    // IOI特殊处理：恢复中国队选手名字
    if(originalComp.name === '世界智能车锦标赛' && originalComp.nationalTeam){
    ...
    const passRate = getPassRateForCompetition(originalComp.name);
      ...
      const totalMax = (config && Array.isArray(config.problems)) ? config.problems.reduce((sum, p) => {
        const lastScore = (Array.isArray(p.subtasks) && p.subtasks.length) ? (p.subtasks[p.subtasks.length - 1].score || 0) : 0;
        const target = Number(p.targetScore || lastScore || 0);
        return sum + target;
      }, 0) : (originalComp && originalComp.maxScore) || 0;
    ...
    if(originalComp.name === '世界智能车锦标赛' && originalComp.nationalTeam){
      ...
      logAt('info', `[IOI金牌线] 前10%分数: ${top10PercentScore}, 标准线: ${standardPassLine}, 最终金牌线: ${passLine}`);
    } else {
    ...
    // medal field: 'gold'|'silver'|'bronze'|null - only used for 全国总决赛 和 世界智能车锦标赛
        let medal = null;
        if(originalComp && originalComp.name === '全国总决赛'){
        ...
        let remark = state._remark || '';
        if(state.trackState){
          const track = state.trackState;
          if(track.disqualified){
            remark = remark ? `${remark}； 违规取消成绩` : '违规取消成绩';
          } else {
            const totalTime = track.finalTimeSec || (track.totalTimeSec + track.penaltySec);
            const mainTime = track.totalTimeSec || 0;
            const penaltyTime = track.penaltySec || 0;
            const summary = `赛道用时${formatSeconds(totalTime)}（净${formatSeconds(mainTime)}${penaltyTime > 0 ? ` + 罚时${formatSeconds(penaltyTime)}` : ''}）`;
            remark = remark ? `${remark}； ${summary}` : summary;
          }
        }
    ...
    // IOI奖牌计算：按排名百分比分配奖牌
    if(originalComp && originalComp.name === '世界智能车锦标赛' && originalComp.nationalTeam){
      try{
        // 只对参赛选手计算奖牌（包括国际选手）
        ...
        logAt('info', `[IOI奖牌分配] 参赛人数:${n}, 金牌:${goldMax}, 银牌:${silverMax}, 铜牌:${bronzeMax}`);
      }catch(e){
  ...
  const halfIndex = (game.week > halfBoundary_local) ? 1 : 0;
    ...
    // 根据比赛等级和成绩计算声誉增加
    if(originalComp.name === '全国总决赛'){
      // 全国总决赛：金奖+8，银奖+5，铜奖+3，晋级+1
      reputationGain = medalCount.gold * 8 + medalCount.silver * 5 + medalCount.bronze * 3 + passedCount * 1;
    } else if(originalComp.name === '分赛区决赛'){
      // 省选：晋级+3
      reputationGain = passedCount * 3;
    } else if(originalComp.name === '区域赛'){
      // 区域赛：晋级+2
      reputationGain = passedCount * 2;
    } else if(originalComp.name === '技术方案评审'){
      // CSP-S2：晋级+1
      reputationGain = passedCount * 1;
    } else if(originalComp.name === '校内选拔赛'){
      // CSP-S1：晋级+1（但系数较低）
    ...
    if(reputationGain > 0){
      ...
      if(typeof window.log === 'function'){
        ...
        } else if(originalComp.name === '世界智能车锦标赛' && originalComp.nationalTeam){
          desc = `获得 🥇${medalCount.gold} 🥈${medalCount.silver} 🥉${medalCount.bronze}`;
        } else {
    ...
    if(game.inNationalTeam && originalComp.name){
      ...
      // CTT 比赛：累计分数
      if(compName === '集训营-硬件冲刺' || compName === '集训营-算法对抗'){
        ...
        logAt('info', `【国家集训队】${compName} 成绩已记录:`, game.nationalTeamResults.cttScores);
      }
      
      // CTS 比赛：记录成绩并计算IOI晋级
      else if(compName === '国家队集训考核'){
        ...
        logAt('info', '【国家集训队】CTS 成绩已记录:', game.nationalTeamResults.ctsScores);
      ...
      // IOI 比赛：计算奖牌并触发结局
      else if(compName === '世界智能车锦标赛'){
        logAt('info', '【国家集训队】IOI 比赛结束，准备计算奖牌和结局');
        // 延迟计算结局（在详细结果弹窗关闭后）
    ...
    if(originalComp.name === '世界智能车锦标赛' && originalComp.nationalTeam){
      try{
        ...
        logAt('debug', '【IOI】保存全部参赛选手成绩到 game.lastIOIAllResults, count=', game.lastIOIAllResults.length);
    ...
    const careerResults = results.filter(r => {
      ...
      if(originalComp.name === '世界智能车锦标赛' && originalComp.nationalTeam && r.student.isInternational) return false;
      return true;
    });
    ...
    logAt('debug', '【DEBUG】 handleCompetitionResults pushed record, careerCompetitions:', game.careerCompetitions);

    // 保存到 localStorage
    if(typeof window.saveGame === 'function') window.saveGame();
    logAt('debug', '【DEBUG】 after saveGame in handleCompetitionResults, localStorage oi_coach_save updated');
  ...
  function getCompetitionCutoff(compName, totalMax){
    const game = window.game;
    if(!totalMax || totalMax <= 0) return 1;
    ...
    // 调试日志
    logAt('debug', `[晋级线计算] 比赛: ${compName}, 省份: ${provinceType}, 总分: ${totalMax}`);
    logAt('debug', `[晋级线计算] cutoffConfig:`, cutoffConfig);
    logAt('debug', `[晋级线计算] compConfig:`, compConfig);
    logAt('debug', `[晋级线计算] basePercentage:`, basePercentage);
    ...
    if(typeof basePercentage !== 'number'){
      ...
      logAt('debug', `[晋级线计算] 使用默认值: ${basePercentage}`);
    }
    
    // 全国总决赛的分数线没有浮动，其他比赛应用浮动（±5%）
    ...
    if(compName !== '全国总决赛'){
      ...
      logAt('debug', `[晋级线计算] 应用浮动后: ${cutoffPercentage}`);
    } else {
      logAt('debug', `[晋级线计算] 全国总决赛不应用浮动，保持: ${cutoffPercentage}`);
    }
    ...
    logAt('info', `[晋级线计算] 最终分数线: ${Math.floor(cutoffScore)} (${(cutoffPercentage * 100).toFixed(1)}%)`);
    
    return Math.floor(Math.max(1, cutoffScore));
  }
  ...
  const seasonIdx = (game.week > halfBoundary_for_update) ? 1 : 0;
    ...
    for(let r of results){
      ...
      if(r.passed && !r.notParticipated){
        ...
        // 日志输出晋级资格记录
        logAt('debug', `[晋级资格记录] 赛季${seasonIdx} ${compName}: ${r.student.name} 晋级`);
      }
    }
    
    // 日志输出当前比赛的晋级人数
    logAt('info', `[晋级资格汇总] 赛季${seasonIdx} ${compName} 晋级人数: ${game.qualification[seasonIdx][compName].size}`);
    
    // 如果有下一场比赛，显示有多少人有资格参加
    if(currentIdx < compOrder.length - 1){
      const nextComp = compOrder[currentIdx + 1];
      logAt('debug', `[晋级链] ${game.qualification[seasonIdx][compName].size} 人有资格参加 ${nextComp}`);
    }
  }
  ...
  function distributeRewards(results, compName){
    ...
    const halfIndex = (game.week > halfBoundary_ui) ? 1 : 0;
      ...
      if(game.fundingIssued.has(fundingKey)){
        logAt('debug', '[distributeRewards] funding already issued for', fundingKey);
        return 0;
      }
    ...
    const modalRoot = document.getElementById('modal-root');
      ...
      try{
        if(modalRoot.children && modalRoot.children.length > 0){
          ...
          // 计划稍后重试，最多重试5次
          if(!window._contestResultRetryCounts) window._contestResultRetryCounts = {};
          const cnt = window._contestResultRetryCounts[resultKey] || 0;
          if(cnt < 5){
            window._contestResultRetryCounts[resultKey] = cnt + 1;
            try{ window._shownContestResults.delete(resultKey); }catch(e){}
            setTimeout(()=>{
              try{ showDetailedResults(results, config, passLine, totalReward, shouldTriggerEnding, endingReason, hasGoldMedal, noiMaxScore, originalComp); }catch(e){}
            }, 400);
          } else {
            try{ window._shownContestResults.delete(resultKey); }catch(e){}
          }
  ...
  // 显示为两个维度的难度：思维难度 / 代码难度（使用该题最后一档的值代表题目总体难度）
    ...
    if(originalComp && originalComp.nationalTeam){
      if(config.name === '集训营-硬件冲刺' || config.name === '集训营-算法对抗' || config.name === '国家队集训考核'){
        html += `<p style="color:#666;">📊 本场成绩将计入总分，最终根据 CTT+CTS 总分决定IOI晋级</p>`;
      } else if(config.name === '世界智能车锦标赛'){
        html += `<p style="color:#666;">🌍 国际智能车奥林匹克竞赛 - 与各国选手同场竞技</p>`;
      }
    } else {
    ...
    // 先显示参赛学生（按排名）
    if(!global.__SC_DETAILS_STORE) global.__SC_DETAILS_STORE = {};
    const storeKey = `${config.name}_${game.week}`;
    if(!global.__SC_DETAILS_STORE[storeKey]) global.__SC_DETAILS_STORE[storeKey] = {};
    ...
    participatedResults.forEach((r, index) => {
      ...
      for(let prob of r.problems){
        ...
        const isNOIOrAbove = compName === '全国总决赛' || compName === '世界智能车锦标赛';
        // 使用 originalComp 来检查 nationalTeam 属性
        const isNationalTeamSelection = originalComp && originalComp.nationalTeam && (compNameOriginal === '集训营-硬件冲刺' || compNameOriginal === '集训营-算法对抗' || compNameOriginal === '国家队集训考核');
        
        logAt('debug', `[结果显示] 比赛: ${compNameOriginal}, nationalTeam: ${originalComp?.nationalTeam}, isNationalTeamSelection: ${isNationalTeamSelection}`);
      ...
      try{ global.__SC_DETAILS_STORE[storeKey][s.name] = r.problems || []; }catch(e){}
  ...
  <td style="font-size:14px;font-weight:bold;">${(isFinite(Number(r.score)) ? Number(r.score).toFixed(1) : '0.0')}</td>
        <td>${resultText}</td>
          <td>${remarkText} ${r.problems && r.problems.length ? `<button class="btn btn-ghost" data-action="sc-details" data-student="${s.name}" data-store="${storeKey}">明细</button>` : ''}</td>
      </tr>`;
    });
    ...
    if(config.name === '国家队集训考核' && originalComp && originalComp.nationalTeam){
      const game = window.game;
    ...
    try{
      const detailButtons = modalRoot.querySelectorAll('button[data-action="sc-details"]');
      detailButtons.forEach(btn => {
        btn.addEventListener('click', function(){
          const studentName = this.getAttribute('data-student');
          const sk = this.getAttribute('data-store');
          let problems = [];
          try{ problems = (global.__SC_DETAILS_STORE && global.__SC_DETAILS_STORE[sk] && global.__SC_DETAILS_STORE[sk][studentName]) || []; }catch(e){}
          if(!problems || !problems.length) return;
          showSmartCarElementDetails(problems, studentName, config.name);
        });
      });
    }catch(e){}
    ...
    try{
      if(typeof window.closeModal === 'function'){
        ...
        window.closeModal = function(){
          ...
          setTimeout(() => {
            try{
              if(window._shownContestResults && resultKey){
                ...
                if(window._contestResultRetryCounts) delete window._contestResultRetryCounts[resultKey];
              }
            }catch(e){ /* ignore */ }
          }, 500);
          ...
          // 检查是否为第二年全国总决赛且有金奖，优先显示国家集训队选择
          if(hasGoldMedal && config && config.name === '全国总决赛'){
            try{
              logAt('info', '[国家集训队检测] 第二年全国总决赛有金奖，设置待选择标志并显示国家集训队选择弹窗');
              // 设置标志，防止周更新时触发赛季结束
          ...
          if(shouldTriggerEnding){
            try{
              logAt('info', '比赛结果确认后触发游戏结局：' + endingReason);
  ...
  function handleMockContestResults(studentStates, config, isPurchased, diffIdx){
    ...
    for(let state of studentStates){
      ...
      if(config.contestType === 'online'){
        ...
        if(diffIdx <= 0){
          gainRatio = CONTEST_GAIN_RATIOS['校内选拔赛'] || gainRatio;
        } else if(diffIdx === 1){
          gainRatio = CONTEST_GAIN_RATIOS['技术方案评审'] || gainRatio;
        } else if(diffIdx === 2){
          gainRatio = CONTEST_GAIN_RATIOS['区域赛'] || gainRatio;
        } else {
          gainRatio = CONTEST_GAIN_RATIOS['分赛区决赛'] || gainRatio;
        }
      }
      ...
      if(window.__OI_DEBUG_MOCK_GAINS){
        logAt('debug', `[模拟赛增幅] 学生:${s.name}`);
        logAt('debug', `  知识上限:${knowledgeCap.toFixed(2)}, 分配:`, knowledgeGains);
        logAt('debug', `  思维上限:${thinkingCap.toFixed(2)}, 分配:`, thinkingGains);
        logAt('debug', `  代码上限:${codingCap.toFixed(2)}, 分配:`, codingGains);
        logAt('debug', `  题目信息:`, problemsWithScores.map(p => ({
  ...
  function showMockContestResults(studentStates, config){
    ...
    // 如果已有模态存在，则延迟显示模拟赛结果，最多重试5次
    try{
      if(modalRoot.children && modalRoot.children.length > 0){
        if(!window._mockResultRetryCounts) window._mockResultRetryCounts = {};
        const cnt = window._mockResultRetryCounts[resultKey] || 0;
        if(cnt < 5){
          window._mockResultRetryCounts[resultKey] = cnt + 1;
          try{ window._shownMockResults.delete(resultKey); }catch(e){}
          setTimeout(()=>{
            try{ showMockContestResults(studentStates, config); }catch(e){}
          }, 400);
        } else {
          try{ window._shownMockResults.delete(resultKey); }catch(e){}
        }
          ...
          setTimeout(() => {
            try{
              if(window._shownMockResults && resultKey){
                ...
                if(window._mockResultRetryCounts) delete window._mockResultRetryCounts[resultKey];
              }
            }catch(e){ /* ignore */ }
          }, 500);
  ...
  function showSmartCarElementDetails(problems, studentName, compName){
    const modalRoot = document.getElementById('modal-root');
    if(!modalRoot) return;
    let rows = '';
    for(let prob of problems){
      const el = prob.trackElement || {};
      const name = el.name || el.type || '元素';
      const solved = prob.solved ? '✓' : '✗';
      const rt = prob.trackRuntime || {};
      const t = rt.timeSec || 0;
      const p = rt.penaltySec || 0;
      const sc = typeof prob.maxScore === 'number' ? prob.maxScore : 0;
      rows += `<tr><td>${name}</td><td style="text-align:center">${solved}</td><td>${formatSeconds(t)}</td><td>${p>0?formatSeconds(p):'—'}</td><td>${sc}</td></tr>`;
    }
    const html = `<div class="modal"><div class="dialog" style="max-width:800px;max-height:80%;overflow:auto">
      <h3>${compName} · ${studentName} 成绩明细</h3>
      <table style="width:100%;font-size:13px;border-collapse:collapse"><thead><tr><th>元素</th><th style="text-align:center">成功</th><th>用时</th><th>罚时</th><th>分数</th></tr></thead><tbody>${rows}</tbody></table>
      <div class="modal-actions" style="margin-top:10px"><button class="btn" onclick="closeModal()">关闭</button></div>
    </div></div>`;
    modalRoot.innerHTML = html;
  }
  ...
  function showCompetitionEndingSummary(comp, results, passLine, passCount, shouldTriggerEnding, endingReason){
    ...
    if(confirmBtn){
      confirmBtn.onclick = function(){
        ...
        if(shouldTriggerEnding){
          try{
            logAt('info', '比赛结束确认后触发游戏结局：' + endingReason);
            if(typeof window.triggerGameEnding === 'function'){
              window.triggerGameEnding(endingReason);
            }
          }catch(e){
